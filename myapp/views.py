...
# 工具/接口测试工具
def interfacetest(request):
    resp1 = "Error: No Response"
    data = json.loads(request.body)
    request_method = data.get("request-method", None)
    interface_url = data.get("request-url", None)
    header = data.get('request-header', None)
    body = data.get('request-body', None)
    if request_method == 'POST':
